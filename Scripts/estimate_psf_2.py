...
def ham3d(sitkIm):
  shp = sitk.GetArrayViewFromImage(sitkIm).shape
  ham_x = scipy.signal.windows.hamming(shp[0])
  ham_y = scipy.signal.windows.hamming(shp[1])
  ham_z = scipy.signal.windows.hamming(shp[2])
  h3d = np.outer(ham_x, ham_y)
  h3d = np.outer(h3d, ham_z).reshape(shp)
  return h3d
  
def estPSF_fourier(ideal, observed):
    h1 = ham3d(ideal)
    h2 = ham3d(observed)
    idealF = scipy.fft.rfftn(h1 * sitk.GetArrayViewFromImage(ideal))
    observedF = scipy.fft.rfftn(h2 * sitk.GetArrayViewFromImage(observed))
    FF = observedF/idealF
    psf = scipy.fft.irfftn(FF)
    res = sitk.GetImageFromArray(np.real(psf))
    res.CopyInformation(ideal)
...
def main():
    ...
    # chop a window out of the middle
    #ideal = ideal[:,:,100:300]
    #axialresampled = axialresampled[:,:,100:300]
