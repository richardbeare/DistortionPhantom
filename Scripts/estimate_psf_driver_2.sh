...
# New version using the averaged and smoothed warp fields to avoi
# the over fitting

# New process is - unwarp->affine registration
...
UNWARPFIELD="NO_UNWARP_FIELD"
BB=$( basename ${AXIAL} )

case "$BB" in 
    *AXI_Fast*)
        UNWARPFIELD=/group/deve2/ACTIVE/data/richard.beare/REVAMP/DistortionPhantomUnWarpBam/T2_AXI_Fast.nii.gz
        ;;
    *AXI_*)
        UNWARPFIELD=/group/deve2/ACTIVE/data/richard.beare/REVAMP/DistortionPhantomUnWarpBam/T2_AXI.nii.gz
        ;;
    *COR_Fast*)
        UNWARPFIELD=/group/deve2/ACTIVE/data/richard.beare/REVAMP/DistortionPhantomUnWarpBam/T2_COR_Fast.nii.gz
        ;;
    *COR_*)
        UNWARPFIELD=/group/deve2/ACTIVE/data/richard.beare/REVAMP/DistortionPhantomUnWarpBam/T2_COR.nii.gz
        ;;
    *SAG_Fast*)
        UNWARPFIELD=/group/deve2/ACTIVE/data/richard.beare/REVAMP/DistortionPhantomUnWarpBam/T2_SAG_Fast.nii.gz
        ;;
    *SAG_*)
        UNWARPFIELD=/group/deve2/ACTIVE/data/richard.beare/REVAMP/DistortionPhantomUnWarpBam/T2_SAG.nii.gz
        ;;
    *)
        echo "no matching unwarpfield"
        exit
        ;;
esac
...
BN=$(basename ${AXIAL})
OP1=${TD}/${BN}
antsApplyTransforms -d 3 -f 0 -i ${AXIAL} -r ${AXIAL} -o ${OP1} -t ${UNWARPFIELD}
